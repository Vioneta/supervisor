...
async def test_start(
    ...
    with (
        ...
        assert (
            run.call_args.args[0] == "ghcr.io/vioneta/qemux86-64-vionetaos"
        )
...
async def test_core_loads_wrong_image_for_machine(
    ...
    coresys.homeassistant.image = "ghcr.io/vioneta/odroid-n2-vionetaos"
    ...
    assert coresys.docker.images.remove.call_args_list[0].kwargs == {
        "image": "ghcr.io/vioneta/odroid-n2-vionetaos:latest",
        "force": True,
    }
    assert coresys.docker.images.remove.call_args_list[1].kwargs == {
        "image": "ghcr.io/vioneta/odroid-n2-vionetaos:2024.4.0",
        "force": True,
    }
    coresys.docker.images.pull.assert_called_once_with(
        "ghcr.io/vioneta/qemux86-64-vionetaos:2024.4.0",
        platform="linux/amd64",
    )
    assert (
        coresys.homeassistant.image == "ghcr.io/vioneta/qemux86-64-vionetaos"
    )
...
async def test_core_loads_wrong_image_for_architecture(
    ...
    coresys.homeassistant.version = AwesomeVersion("2024.4.0")
    container.attrs["Config"] = {"Labels": {"io.hass.version": "2024.4.0"}}
    coresys.docker.images.get("ghcr.io/vioneta/qemux86-64-vionetaos").attrs[
        "Architecture"
    ] = "arm64"
    ...
    container.remove.assert_called_once_with(force=True)
    assert coresys.docker.images.remove.call_args_list[0].kwargs == {
        "image": "ghcr.io/vioneta/qemux86-64-vionetaos:latest",
        "force": True,
    }
    assert coresys.docker.images.remove.call_args_list[1].kwargs == {
        "image": "ghcr.io/vioneta/qemux86-64-vionetaos:2024.4.0",
        "force": True,
    }
    coresys.docker.images.pull.assert_called_once_with(
        "ghcr.io/vioneta/qemux86-64-vionetaos:2024.4.0",
